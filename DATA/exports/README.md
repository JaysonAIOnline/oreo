...
**Regenerated by:** `DATA/sync_neo4j_to_oreo.py` (run from repo root with `/root/.venv/bin/python`)

> 2026-09-07/08 data-integrity cleanup: the datastore held duplicate source rows
> for the same URL (29 dup URLs), so facts were double-written (285 rows for 139
> unique fact+URL pairs). Duplicate sources/facts/sections were deduped in the
> DB; `oreo_research_complete.csv` previously contained a SQL error
> (`column f.section_id does not exist`) and now exports one row per fact.

**Current content:** 139 facts, 29 sources, 29 sections, 12 categories, 30 subcategories
...
|------|------|-------------|
| `facts_by_category.csv` | 151 | All facts with category, content, confidence, fact_type, key_term |
| `sources_categories.csv` | 29 | Sources mapped to categories and subcategories |
| `oreo_research_complete.csv` | 140 | Full export — one row per fact (key_term, fact_type, value, confidence, source_url, source_title) |
